...
- **TOP 100 카드보드**(`/kospi-100`, `/kosdaq-100`, `/nasdaq-100`): 각 시장 시가총액 상위 100종목을 **업종별로 그룹핑한 카드 UI**. 메인 허브의 **블랙홀**을 클릭하면 진입하고, 세 페이지는 헤더의 세그먼트 탭으로 서로 오갑니다. 카드 한 장이 네 개의 시간축을 동시에 답합니다 — 오늘(등락 배지), 3개월(스파크라인, 시작→끝 방향으로 색이 결정되고 마우스를 올리면 그 날짜의 종가를 읽어줌), 52주(현재가 위치 마커가 붙은 레인지 트랙), 그리고 1주/1개월/3개월/연초 수익률 칩. 여기에 **시장 대비 초과등락(%p)과 업종 내 당일 순위**, 국내 종목은 거래량·PER·ROE·외국인비율이 함께 붙습니다. 업종 헤더/칩은 시총가중 평균 등락을 트리맵과 같은 발산형 색상으로 칠해, 클릭 전에도 "오늘 어느 업종이 좋았나"가 먼저 읽힙니다. 검색·정렬(시총/상승률/하락률/거래량/52주 고점근접/1개월수익률)·업종 필터·업종별↔전체순위·카드↔간략히 전환 지원, 모바일 1열 최적화
...
- **지수 차트**: 코스피/코스닥 지수 차트
- **AI 종목예측**(`/ai-prediction`): 코스피 시총 1~30위, 코스닥 시총 1~30위, 나스닥 시총 1~30위 + AMD·인텔·마이크론·샌디스크에 대해 장 마감 직후 배치가 익일 시세·방향을 예측. 단순 방향이 아니라 **상승/보합/하락 확률, 예측 신뢰도, 당일 장 마감 원인 설명, 실제 사용된 근거 데이터, 예측 대비 실제 적중률**을 함께 기록해 예측의 이유와 성과를 검증할 수 있게 합니다. 날짜 네비게이터로 과거 예측과 그 채점 결과를 넘겨볼 수 있음
...
- **네이버 금융** 크롤링: 실시간 시세, 뉴스, 종목토론방, 10호가 창, 투자자 매매동향, 일봉 시계열(`siseJson`, TOP 100 보드의 스파크라인·52주 범위용)
...
- **배치 트리거**: `POST /api/prediction/run?region=KR|US`(`Authorization: Bearer $PREDICTION_BATCH_TOKEN`). 한국장(코스피·코스닥)은 **평일 23:00 KST**, 미국장(나스닥)은 **평일 23:00 ET**에 각자 그날 마감된 세션을 대상으로 독립 실행하고, 여기에 **일요일 22:00**(각 시장 현지시각) 슬롯이 하나씩 더 붙습니다 — 일요일에는 새로 마감된 세션이 없어 `has_run`에 걸리므로 이 슬롯만 `force=true`로 **반드시 실행**되어 금요일 세션을 재수집·재채점합니다(같은 `(수집일자, 종목코드)`에 upsert이므로 행이 늘지 않고 갱신). **토요일은 두 시장 모두 실행하지 않습니다.** 다음 개장 거래일은 주말·공휴일을 반영한 `trading_calendar.py`가 계산합니다. 1차 트리거는 GitHub Actions cron(`.github/workflows/ai-prediction.yml`, 미국장은 서머타임 때문에 슬롯마다 UTC 두 줄), 2차 안전망은 백엔드 인프로세스 스케줄러(`prediction_batch.start_scheduler`) — 둘 다 idempotent한 같은 `run_batch`를 호출하므로 먼저 도달한 쪽이 저장하고 다른 쪽은 skip합니다.
...
| GET | `/api/market/board?market=kospi\|kosdaq\|nasdaq` | TOP 100 카드보드 (업종 요약 + 종목별 스파크라인·52주 범위·기간수익률) |
