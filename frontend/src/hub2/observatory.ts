...
export const OBSERVABLES: ObservableSpec[] = [
  ...
  },
  {
    ...
    en: "Wormhole",
    group: "exotic",
    /* Framed for the lensing, not for the sphere. The ring of bent sky reaches
       to about seven of its radii; framed on the ball alone the one thing that
       makes it a wormhole rather than a marble would sit off every edge. */
    frame: 9,
    radiusKm: "관측된 예 없음 — 이론상의 천체",
    mass: "음의 에너지 밀도를 요구",
    density: "정의되지 않음",
    temperature: "해당 없음",
    description:
      "아인슈타인과 로젠이 1935년 블랙홀의 방정식을 풀다 발견한 구조입니다. 시공간의 서로 다른 두 지점을 잇는 통로이며, 두 입구 사이의 거리가 바깥 공간을 가로지르는 거리보다 짧을 수 있습니다. 종이 위의 두 점을 잇는 가장 짧은 길이 종이를 접어 뚫는 것인 것과 같은 이치입니다.\n\n방정식이 허용한다는 것과 존재한다는 것은 다릅니다. 계산상 웜홀은 만들어지자마자 자기 중력으로 목이 졸려 닫히며, 빛조차 통과할 시간이 없습니다. 열어 두려면 음의 에너지 밀도를 가진 물질 — 흔히 이색 물질이라 불리는 것 — 이 필요한데, 그런 물질이 거시적인 양으로 존재한다는 증거는 아직 없습니다.\n\n입구는 깔때기가 아니라 구입니다. 흔히 그려지는 깔때기 그림은 차원 하나를 줄여 단면을 나타낸 도해일 뿐이고, 실제로 옆에서 본다면 반대편 하늘이 통째로 비쳐 드는 공처럼 보입니다. 가장자리로 갈수록 상이 무한히 압축되어 밝은 테를 이루는데, 블랙홀의 광자 고리가 밝은 것과 같은 이유입니다.\n\n영화 인터스텔라의 웜홀은 킵 손이 세운 방정식을 그대로 렌더링해 만들었고, 그 과정에서 나온 계산은 실제 논문으로 발표되었습니다. 토성 근처에 놓인 것도 그 설정을 따른 것입니다.",
  },
];
