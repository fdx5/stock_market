...
import { StockSearchResult, Top100LivePrice, Top100PredictionItem, api } from "../api/client";
...
export default function Top100PredictionPanel({
  ...
  const [prices, setPrices] = useState<Record<string, Top100LivePrice>>({});
  ...
  useEffect(() => {
    ...
    // The backend caches per rank tier internally (1-20위 10초 · 21-50위 2분 · 51-100위
    // 5분), so polling at the fastest tier here is enough to pick up every tier's
    // refresh the moment it happens — no need to mirror the tiering client-side too.
    const loadPrices = () => {
      api
        .top100Prices()
        .then((res) => {
        ...
        .catch(() => {
          // A missed price refresh just keeps showing the last known values.
        });
    };
    ...
    <section className="card top100-panel">
      <div className="top100-header">
        ...
        <span className="kospi-map-live-badge">
          ...
          실시간 (1~20위 10초 · 21~50위 2분 · 51~100위 5분 갱신)
        </span>
      </div>
      <p className="top100-subtitle">
        전일 종가까지의 데이터로 산출한 다음 거래일 방향성입니다. 현재가·등락은 실시간 반영됩니다. 종목명을 누르면
        예측 결과를 바로 조회하고, 신뢰도 옆 이력 버튼을 누르면 예측 적중 이력으로 이동합니다.
      </p>
      ...
      {!loading && !error && (
        <div className="top100-table-wrap">
          <table className="top100-table">
            ...
            <tbody>
              {items.map((item) => {
                const live = prices[item.code];
                const price = live?.close ?? item.last_close;
                ...
                return (
                  <tr key={item.code}>
                    ...
                    <td className="col-price">{price.toLocaleString()}원</td>
                    <td
                      className="col-change"
                      style={{ color: changePct === undefined ? "var(--text-muted)" : changePct >= 0 ? "var(--up-color)" : "var(--down-color)" }}
                    >
