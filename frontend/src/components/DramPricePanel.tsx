...
// 반도체/전자 종목의 상세 페이지에서만 의미가 있는 패널이라, 매번 종목이 바뀔 때만
// 다시 확인하면 충분하다 — 배치는 하루 한 번만 갱신되므로 폴링은 두지 않는다.
const SEMICONDUCTOR_SECTOR = "반도체/전자";

/** D램 현물가격 패널: 선택된 종목의 업종이 반도체/전자일 때만, 일일 배치가 마지막으로
 * 적재한 TrendForce 현물가 스냅샷을 항목별 표로 보여준다. 추이 그래프는 날짜별 데이터가
 * 충분히 쌓인 뒤 추가할 예정이라 지금은 최신 스냅샷 리스트만 그린다. */
export default function DramPricePanel({ code }: { code: string }) {
  const t = useT();
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        if (res.sector !== SEMICONDUCTOR_SECTOR) {
          setLoaded(true);
          return;
        }
        api
  ...
  if (!loaded || sector !== SEMICONDUCTOR_SECTOR || items.length === 0) return null;
