...
export default function UsStockIntelligencePage({ code, isEtf = false }: { code: string; isEtf?: boolean }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      isEtf ? Promise.resolve(null) : api.globalEnrichment(ticker, "ko"), api.usStockQuote(ticker),
  ...
  return <div className="si-page si-page--us">
    ...
    {quote && <main>
      <section className="si-hero">
        <div className="si-identity">{enrichment?.logo_url ? <img className="si-logo si-global-logo" src={enrichment.logo_url} alt="" /> : <span className="si-ticker-logo">{ticker.slice(0, 2)}</span>}<div><span className="si-market">US {isEtf ? "ETF" : "MARKET"} · {latest?.date ?? "LATEST"}</span><h1>{quote.name}</h1><p>{ticker}</p></div></div>
        ...
        <div className="si-hero-action"><button onClick={() => document.getElementById("si-chart")?.scrollIntoView({ behavior: "smooth" })}>종합 분석 보기 ↓</button><Link to={`/discussion-explorer?code=${ticker}&name=${encodeURIComponent(quote.name)}&market=US&asset=${isEtf ? "ETF" : "STOCK"}`}>토론 바로가기</Link><MarketBubbleStockLink code={ticker} market="nasdaq" /></div>
      </section>
      <div className="si-talk-ticker"><DiscussionHeadlineTicker code={ticker} market="US" limit={30} asset={isEtf ? "ETF" : "STOCK"} /></div>
      <section className="si-signal-grid" aria-label="해외 종목 핵심 지표">
        ...
        <article><span>{isEtf ? "자산 유형" : "시가총액"}</span><strong>{isEtf ? "ETF" : cap(enrichment?.marcap_usd)}</strong><small>{isEtf ? "미국 상장지수펀드" : "미국 달러 기준"}</small></article>
      ...
      {!isEtf && <section className="si-section"><header><div><span>SECTOR CONTEXT</span><h2>동일 업종 비교</h2></div><p>S&amp;P 500 동일 섹터 종목</p></header><div className="si-panel si-panel--map"><UsSectorMapPanel code={ticker} onSelectStock={stock => navigate(`/stock/${stock.code}`)} /></div></section>}
