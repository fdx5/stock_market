...
export default function SearchLandingPage({ kind, code, slug, codes }: { kind: Kind; code?: string; slug?: string; codes?: string[] }) {
  ...
  const meta = useMemo(() => {
    ...
    if (kind === "investor-ranking") return ["종목별 외국인·기관 수급 전체보기", "코스피·코스닥 상장 종목의 외국인·기관·개인 순매수 금액을 비교하고 종목별 투자자 매매동향 페이지로 이동하세요."];
