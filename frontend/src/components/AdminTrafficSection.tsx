...
/* 관리자 콘솔의 트래픽 화면 — 요약 지표, 방문 추이 차트, 페이지·종목·메인 반응 순위.
 * 콘솔 셸(AdminDashboardPage)이 메뉴와 상태 표시줄을 갖고, 이 파일은 트래픽 데이터만
 * 소유한다. 각 묶음(요약, 추이, 순위)은 따로 불러오고 따로 실패한다: 하나가 늦거나
 * 실패해도 나머지는 그리고, 실패한 곳은 마지막 값과 오류, 재시도 버튼을 보인다. */
...
/** What went wrong loading one group of figures, and when it last loaded. */
interface LoadState {
  error: string | null;
  at: Date | null;
}
...
function LoadNote({ state, onRetry, what }: { state: LoadState; onRetry: () => void; what: string }) {
  if (!state.error) return null;
  return (
    <div className="admin-load-error" role="alert">
      <span>
        {what}을(를) 불러오지 못했습니다 — {state.error}
        {state.at && ` · ${state.at.toLocaleTimeString("ko-KR", { hour12: false })} 값 표시 중`}
      </span>
...
export default function AdminTrafficSection() {
  ...
  const [summaryState, setSummaryState] = useState<LoadState>({ error: null, at: null });
  const [trendState, setTrendState] = useState<LoadState>({ error: null, at: null });
  const [rankState, setRankState] = useState<LoadState>({ error: null, at: null });
  const [reload, setReload] = useState({ summary: 0, trend: 0, rank: 0 });
    ...
    const load = () => {
      adminApi
        ...
        .then((s) => {
          if (cancelled) return;
          setSummary(s);
          setSummaryState({ error: null, at: new Date() });
        })
        .catch((err) => {
          if (cancelled || handleAuthError(err)) return;
          setSummaryState((prev) => ({ ...prev, error: errorText(err) }));
        });
    };
    load();
  ...
  }, [authed, reload.summary]);

  useEffect(() => {
    if (!authed) return undefined;
    let cancelled = false;
    const load = () => {
      // Each series on its own: one slow or failing aggregate used to hold the whole
      // chart on "loading" for good (Promise.all never settled into a render).
      Promise.allSettled([adminApi.trend(range), adminApi.visitorTrend(range), adminApi.hubTrend(range), adminApi.hubSummary(range)]).then(
        ([pages, visitors, hub, hubStats]) => {
          if (cancelled) return;
          const failed = [pages, visitors, hub, hubStats].find((r) => r.status === "rejected") as PromiseRejectedResult | undefined;
          if (failed && handleAuthError(failed.reason)) return;
          if (pages.status === "fulfilled") setTrendPoints(pages.value.points);
          if (visitors.status === "fulfilled") setVisitorPoints(visitors.value.points);
          if (hub.status === "fulfilled") setHubPoints(hub.value.points);
          if (hubStats.status === "fulfilled") setHubSummary(hubStats.value);
          setTrendLoaded(true);
          setTrendState((prev) => (failed ? { ...prev, error: errorText(failed.reason) } : { error: null, at: new Date() }));
        },
      );
    };
    load();
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [authed, range, reload.trend]);

  useEffect(() => {
    if (!authed) return undefined;
    let cancelled = false;
    const load = () => {
      Promise.allSettled([adminApi.pagesTop(200), adminApi.stocksTop(500), adminApi.hubObjectsTop(300), adminApi.bubbleStats()]).then(
        ([pages, stocks, hub, bubbles]) => {
          if (cancelled) return;
          const failed = [pages, stocks, hub, bubbles].find((r) => r.status === "rejected") as PromiseRejectedResult | undefined;
          if (failed && handleAuthError(failed.reason)) return;
          // A ranking that failed shows as empty rather than as a skeleton forever.
          setPagesTop((prev) => (pages.status === "fulfilled" ? pages.value.items : (prev ?? [])));
          setStocksTop((prev) => (stocks.status === "fulfilled" ? stocks.value.items : (prev ?? [])));
          setHubObjects((prev) => (hub.status === "fulfilled" ? hub.value.items : (prev ?? [])));
          if (bubbles.status === "fulfilled") setBubbleStats(bubbles.value);
          setRankState((prev) => (failed ? { ...prev, error: errorText(failed.reason) } : { error: null, at: new Date() }));
        },
      );
    };
    load();
  ...
  return (
    <div className="admin-traffic">
      <LoadNote state={summaryState} what="요약 지표" onRetry={() => setReload((r) => ({ ...r, summary: r.summary + 1 }))} />
      <LoadNote state={trendState} what="방문 추이" onRetry={() => setReload((r) => ({ ...r, trend: r.trend + 1 }))} />
      <LoadNote state={rankState} what="순위" onRetry={() => setReload((r) => ({ ...r, rank: r.rank + 1 }))} />
