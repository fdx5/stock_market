...
/* The live "실시간 세션 / 실시간 로그" view — session chip strip, a scope filter,
 * and a pausable/resumable event list with per-session trail expansion.
 *
 * Extracted from AdminLivePage.tsx so the mobile console at /admin/live and the
 * desktop dashboard's 실시간 tab share one implementation instead of two drifting
 * copies. Each caller supplies only its own surrounding chrome (topbar vs. card
 * heading) — everything about polling, pausing, and rendering rows lives here.
 */
...
export default function LiveSessionsAndLog({
  className,
  onStatus,
}: {
  className?: string;
  /** Fired whenever the connection status changes, so a caller's own header (the
   * mobile topbar's pulse dot, the dashboard tab's status line) can show it
   * without this component owning any page-level chrome itself. */
  ...
  useEffect(() => {
    onStatus?.({ lastUpdated, connectionOk });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lastUpdated, connectionOk]);
  ...
  return (
    <div className={className ? `admin-live-embed ${className}` : "admin-live-embed"}>
